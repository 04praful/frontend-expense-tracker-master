...
const ProfileForm = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://hello-a7fb5-default-rtdb.firebaseio.com//user/${ChangesEMail}.json`
      )
      .then((response) => {
  ...
  const submitHandler = (e) => {
    ...
    if (userData === null) {
      ...
      axios
        .put(
          `https://hello-a7fb5-default-rtdb.firebaseio.com//user/${ChangesEMail}/${Object.keys(userData)[0]}.json`,
          { name: enteredName, city: enteredCity, dob: enteredDob }
        )
        .then((response) => {
          console.log(response.data);
